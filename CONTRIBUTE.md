...
SolidStart のアセット出力先は `_build/` ディレクトリ（アンダースコア始まり）。GitHub Pages はデフォルトで Jekyll を通してサイトをビルドするが、Jekyll は `_` で始まるディレクトリを無視する。その結果、CSS・JS が 404 になる。

`docs/.nojekyll`（空ファイル）を置くことで Jekyll 処理をスキップし、`docs/` の内容をそのまま配信させる。`build.sh` で自動生成される。
